...
struct PlaceCreationView: View {
    ...
    private var searchSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            TextField("Search for a place or business...", text: $viewModel.searchQuery)
            ...
            if viewModel.searchResults.isEmpty {
                statusCard(viewModel.searchQuery.trimmingCharacters(in: .whitespaces).isEmpty
                    ? "Type a name or address above."
                    : "No results — try a different name or suburb.")
            } else {
