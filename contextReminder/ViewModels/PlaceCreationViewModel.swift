...
final class PlaceCreationViewModel: ObservableObject {
    ...
    func performSearch() {
        ...
        // Apply the user's current location as a region bias so that queries
        // like "Coles" return nearby Australian results rather than global ones.
        if let mkSearcher = searcher as? MKLocalAddressSearcher,
           let coord = location.currentCoordinate {
            let centre = CLLocationCoordinate2D(
                latitude: coord.latitude,
                longitude: coord.longitude
            )
            mkSearcher.regionBias = MKCoordinateRegion(
                center: centre,
    ...
    /// Called from the view's `.onChange(of: searchQuery)`. Waits 400 ms after
    /// the user stops typing before firing, so we don't hammer the API on every
    /// keystroke.
    func scheduleSearch() {
        searchTask?.cancel()
        let query = searchQuery
        ...
        searchTask = Task { [weak self] in
            guard let self else { return }
            // Debounce: wait before firing so rapid keystrokes don't each
