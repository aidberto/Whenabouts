...
protocol AddressSearching {
    // Look up places matching `query`. Returns up to ~10 suggestions. Returns an empty list if the query is empty or something goes wrong.
...
final class MKLocalAddressSearcher: AddressSearching {
    func search(query: String) async -> [AddressSuggestion] {
        ...
        do {
            ...
            // Convert each result into our own AddressSuggestion type. Cap at 10 so the user isn't overwhelmed by suggestions.
